...
export default function App() {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background selection:bg-primary-container selection:text-primary">
      {/* Top App Bar */}
      <header className="sticky top-0 w-full z-50 bg-background/80 backdrop-blur-xl shadow-[0_12px_40px_rgba(59,45,0,0.08)] flex justify-between items-center px-6 md:px-8 py-4">
          ...
          {isSidebarOpen && (
            <>
              ...
              <motion.aside 
                ...
                animate={{ x: 0 }}
                exit={{ x: '-100%' }}
                className="fixed left-0 top-0 h-full w-64 bg-background z-50 p-4 lg:hidden shadow-2xl"
              >
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 bg-background/80 backdrop-blur-xl border-t border-primary/10 h-20 flex justify-around items-center px-4 z-40">
        {filters.map((filter) => (
          <button 
            key={filter.name}
            onClick={() => setActiveFilter(filter.name)}
...
function StudentCard({ student, index }: StudentCardProps) {
  return (
    ...
    >
      <div className="absolute -top-10 w-24 h-24 rounded-full border-4 border-background overflow-hidden shadow-xl group-hover:scale-110 transition-transform duration-300">
        <img 
